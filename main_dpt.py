...
def test(args, model: DiffTalkingHead, style_enc: Optional[StyleEncoder], test_loader, current_iter, n_rounds=10,
         mode='val', writer=None, flame=None, save_pred_coefs: Optional[Path]=None):
    is_training = model.training

    all_pred = []   # to store predicted coeffs
    all_ids  = []   # if you want to track sample IDs
    ...
    for test_round in range(n_rounds):
        for audio_pair, coef_pair, audio_stats in test_loader:
            ...
            if save_pred_coefs is not None:
                # `target` is shape [N, n_prev + n_motions, D]
                batch_pred = target.detach().cpu().numpy()
                all_pred.append(batch_pred)
                # if your loader gives you example‐IDs in audio_stats, you can do:
                #    ids = audio_stats["id"]    # or whatever key
                #    all_ids.extend(ids)
        ...
        # ─── after all batches: save out .npz ─────────────────────────────────
    if save_pred_coefs is not None and len(all_pred)>0:
        pred_array = np.concatenate(all_pred, axis=0)
        save_path  = Path(save_pred_coefs)
        save_path.parent.mkdir(parents=True, exist_ok=True)
        # key name “pred” is arbitrary but clear
        np.savez(str(save_path), pred=pred_array)
        print(f'>>> saved predicted coeffs to {save_path}')
...
def main(args, option_text=None):
    ...
    if args.mode == 'train':
        ...
        # Test the model
        test(model_args, model, style_enc, test_loader, args.iter, 200, 'test', None, flame, save_pred_coefs=args.save_pred_coefs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_pred_coefs', type=Path, default=None, help='(test only) path to save predicted motion coefficients as .npz')
